...
wkohc(document).ready(function () {
  ...
  wkohc('body').on('click', '#wkohc-add-attachment', function(){
    ...
    html +=   `<td>`;
    // JS edit: Fix inability to send uploaded attachment in messages and field for filename. Step 2: Fix JS
    ...
    // JS edit: Fix inability to send uploaded attachment in messages and field for filename. Step 3: Hide empty filename box
    html +=      `<input type="text" class="wkohc_file_name" disabled style="width:174px;opacity:0"/>`;
    html +=    `</div>`;
  ...
  wkohc('body').on('change', '#wkohc-tbody .wkohc_file', function(){
    ...
    // JS edit: Fix inability to send uploaded attachment in messages and field for filename. Step 4: Show filename
    wkohc(this).parents('td').find('.wkohc_file_name').css('opacity','1').val(name);
  });
