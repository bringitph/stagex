		...
		if ( ! $singular ) {
			...
			<div class="yith-wcwl-add-button" style="display: <?php echo esc_attr( $exists ? 'none' : 'block' ); ?>">
				<a href="<?php echo esc_url( add_query_arg( 'add_to_wishlist', $product->get_id() ) ); ?>"
					...
					if ( ! $singular ) {
						?>
						thb-icon-<?php } ?>text"><?php echo esc_html__( 'Add To Wishlist', 'overline' ); ?></span><i class="thb-icon-favorite"></i>
				</a>
			</div>
			<div class="yith-wcwl-wishlistexistsbrowse">
				<a href="<?php echo esc_url( $url ); ?>">
					...
					if ( ! $singular ) {
						?>
						thb-icon-<?php } ?>text"><?php echo esc_html__( 'View Wishlist', 'overline' ); ?></span><i class="thb-icon-heart"></i>
				</a>
			</div>
		</div>
		<?php
...
function thb_woocommerce_widget_shopping_cart_button_view_cart() {
	echo '<a href="' . esc_url( wc_get_cart_url() ) . '" class="button transparent x-large">' . esc_html__( 'View cart', 'overline' ) . '<span>' . wp_kses_post( thb_load_template_part( 'assets/img/svg/next_arrow.svg' ) ) . '</span></a>';
}

remove_action( 'woocommerce_widget_shopping_cart_buttons', 'woocommerce_widget_shopping_cart_proceed_to_checkout', 20 );

add_action( 'woocommerce_widget_shopping_cart_buttons', 'thb_woocommerce_widget_shopping_cart_proceed_to_checkout', 10 );
function thb_woocommerce_widget_shopping_cart_proceed_to_checkout() {
	echo '<a href="' . esc_url( wc_get_checkout_url() ) . '" class="button checkout wc-forward black x-large">' . esc_html__( 'Checkout', 'overline' ) . '<span>' . wp_kses_post( thb_load_template_part( 'assets/img/svg/next_arrow.svg' ) ) . '</span></a>';
}
