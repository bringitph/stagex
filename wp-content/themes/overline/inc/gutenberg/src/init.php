...
function thb_woocommerce_category_grid_render( $attributes ) {
	...
	ob_start();
	if ( ! thb_wc_supported() ) {
		esc_html_e( 'Please install & activate WooCommerce', 'overline' );
		return;
	}
	?>
	<div class="row products thb-product-category-grid">
		...
		if ( count( $attributes['categories'] ) < 1 ) {
			?>
				...
				<?php esc_html_e( 'Please assign WooCommerce categories', 'overline' ); ?>
				</div>
				<?php
		} else {
...
function thb_block_category( $categories, $post ) {
	return array_merge(
		...
		array(
			array(
				'slug'  => 'fuel-themes',
				'title' => esc_html__( 'Fuel Themes Blocks', 'overline' ),
			),
		)
	);
}
