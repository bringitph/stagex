...
class Thb_Theme_Admin {
	...
	public function thb_after_setup_theme() {
		...
		// Text Domain.
		load_theme_textdomain( 'overline', get_stylesheet_directory() . '/inc/languages' );
		...
		register_nav_menus(
			array(
				'nav-menu'     => esc_html__( 'Navigation Menu', 'overline' ),
				'acc-menu-out' => esc_html__( 'Secondary Menu (Not Logged-in)', 'overline' ),
				'acc-menu-in'  => esc_html__( 'Secondary Menu (Logged-in)', 'overline' ),
			)
		);
		...
		$thb_sidebars = array(
			array(
				'name'        => esc_html__( 'Article Sidebar', 'overline' ),
				'id'          => 'single',
				'description' => esc_html__( 'The sidebar next to articles', 'overline' ),
			),
			array(
				'name'        => esc_html__( 'Blog Sidebar', 'overline' ),
				'id'          => 'blog',
				'description' => esc_html__( 'The sidebar on blog pages', 'overline' ),
			),
			array(
				'name'        => esc_html__( 'Page Sidebar', 'overline' ),
				'id'          => 'page',
				'description' => esc_html__( 'The sidebar on regular pages.', 'overline' ),
			),
			array(
				'name'        => esc_html__( 'Mobile Menu Sidebar', 'overline' ),
				'id'          => 'mobile-menu',
				'description' => esc_html__( 'You can also add widgets inside your mobile menu.', 'overline' ),
			),
		);
		for ( $x = 1; $x <= 6; $x++ ) {
			$thb_sidebars[] = array(
				'name'        => esc_html__( 'Footer Column - ', 'overline' ) . $x,
				'id'          => 'footer' . $x,
				'description' => esc_html__( 'Contents of the column #', 'overline' ) . $x,
			);
		}
		...
		if ( thb_wc_supported() ) {
			array_unshift(
				$thb_sidebars,
				array(
					'name'        => esc_html__( 'Shop Sidebar', 'overline' ),
					'id'          => 'thb-shop-filters',
					'description' => esc_html__( 'Sidebar used for filters on the Shop page', 'overline' ),
				)
			);
		}
		// Register Sidebars.
	...
	public function thb_image_sizes() {
		$thb_image_sizes = apply_filters(
			...
			array(
				array(
					'slug'   => 'overline-rectangle',
					'width'  => 370,
					'height' => 240,
					'crop'   => true,
				),
				array(
					'slug'   => 'overline-single',
					'width'  => 800,
					'height' => 400,
					'crop'   => true,
				),
	...
	public function thb_demos() {
		return array(
			array(
				'import_file_name'           => 'Overline',
				'import_file_url'            => 'https://themes.fuelthemes.net/theme-demo-files/overline/democontent.xml',
				'import_widget_file_url'     => 'https://themes.fuelthemes.net/theme-demo-files/overline/widget_data.json',
				'import_customizer_file_url' => 'https://themes.fuelthemes.net/theme-demo-files/overline/customizer.dat',
				'import_image'               => self::$thb_theme_directory_uri . 'screenshot.jpg',
				'import_demo_url'            => 'https://overline.fuelthemes.net/',
			),
		);
	}
	/**
	...
	public function thb_editor_styles() {
		...
		$secondary_font['font-family'] = '' === $secondary_font['font-family'] ? '-apple-system,BlinkMacSystemFont,Segoe UI,Helvetica,Arial,sans-serif,Apple Color Emoji,Segoe UI Emoji' : $secondary_font['font-family'];
		?>
		<?php if ( $primary_font ) { ?>
		...
		.editor-styles-wrapper .wp-block-latest-posts.is-grid a {
			<?php
