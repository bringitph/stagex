...
class WXRImporter extends \WP_Importer {
	...
	protected function get_reader( $file ) {
		...
		if ( ! $status ) {
			return new \WP_Error( 'wxr_importer.cannot_parse', __( 'Could not open the file for parsing', 'overline' ) );
		}
			...
			switch ( $reader->name ) {
				case 'wp:wxr_version':
					...
					if ( version_compare( $this->version, self::MAX_WXR_VERSION, '>' ) ) {
						$this->logger->warning(
							sprintf(
								__( 'This WXR file (version %1$s) is newer than the importer (version %2$s) and may not be supported. Please consider updating.', 'overline' ),
								$this->version,
	...
	protected function import_start( $file ) {
		if ( ! is_file( $file ) ) {
			return new \WP_Error( 'wxr_importer.file_missing', __( 'The file does not exist, please try again.', 'overline' ) );
		}
	...
	public function set_user_mapping( $mapping ) {
		foreach ( $mapping as $map ) {
			if ( empty( $map['old_slug'] ) || empty( $map['old_id'] ) || empty( $map['new_id'] ) ) {
				$this->logger->warning( __( 'Invalid author mapping', 'overline' ) );
				...
				case 'wp:status':
					...
					if ( $data['post_status'] === 'auto-draft' ) {
						...
						return new \WP_Error(
							...
							__( 'Cannot import auto-draft posts', 'overline' ),
							$data
						);
					}
					break;
	...
	protected function process_post( $data, $meta, $comments, $terms ) {
		...
		if ( ! $post_type_object ) {
			$this->logger->warning(
				sprintf(
					__( 'Failed to import "%1$s": Invalid post type %2$s', 'overline' ),
					$data['post_title'],
					$data['post_type']
				)
			);
		...
		if ( $post_exists ) {
			$this->logger->info(
				sprintf(
					__( '%1$s "%2$s" already exists.', 'overline' ),
		...
		if ( 'attachment' === $postdata['post_type'] ) {
			if ( ! $this->options['fetch_attachments'] ) {
				$this->logger->notice(
					sprintf(
						__( 'Skipping attachment "%s", fetching attachments disabled', 'overline' ),
						$data['post_title']
					)
				);
				return false;
		...
		if ( is_wp_error( $post_id ) ) {
			$this->logger->error(
				sprintf(
					__( 'Failed to import "%1$s" (%2$s)', 'overline' ),
					$data['post_title'],
					$post_type_object->labels->singular_name
				)
			);
		...
		$this->logger->debug(
			sprintf(
				__( 'Post %1$d remapped to %2$d', 'overline' ),
				$original_id,
				$post_id
			)
		);
					...
					if ( 'post_format' === $taxonomy ) {
						...
						if ( empty( $term_id ) ) {
							...
							if ( ! is_wp_error( $t ) ) {
								...
								$this->logger->warning(
									sprintf(
										esc_html__( 'Failed to import term: %1$s - %2$s', 'overline' ),
										esc_html( $taxonomy ),
										esc_html( $term['name'] )
									)
								);
	...
	protected function process_attachment( $post, $meta, $remote_url ) {
		...
		if ( ! $info ) {
			return new \WP_Error( 'attachment_processing_error', __( 'Invalid file type', 'overline' ) );
		}
	...
	protected function process_author( $data, $meta ) {
		...
		if ( is_wp_error( $user_id ) ) {
			$this->logger->error(
				sprintf(
					__( 'Failed to import user "%s"', 'overline' ),
					$userdata['user_login']
				)
			);
		...
		$this->logger->info(
			sprintf(
				__( 'Imported user "%s"', 'overline' ),
				$userdata['user_login']
			)
		);
		$this->logger->debug(
			sprintf(
				__( 'User %1$d remapped to %2$d', 'overline' ),
				$original_id,
				$user_id
			)
		);
	...
	protected function process_term( $data, $meta ) {
		...
		$this->logger->debug(
			sprintf(
				__( 'Term %1$d remapped to %2$d', 'overline' ),
				$original_id,
				$term_id
			)
		);
		...
		foreach ( $meta as $meta_item ) {
			...
			if ( $key ) {
				...
				if ( is_wp_error( $result ) ) {
					$this->logger->warning(
						sprintf(
							__( 'Failed to add metakey: %1$s, metavalue: %2$s to term_id: %3$d', 'overline' ),
							$key,
							$value,
							$term_id
						)
					...
					$this->logger->debug(
						sprintf(
							__( 'Meta for term_id %1$d : %2$s => %3$s ; successfully added!', 'overline' ),
							$term_id,
							$key,
							$value
						)
	...
	protected function fetch_remote_file( $url, $post ) {
		...
		if ( $code !== 200 ) {
			...
			return new \WP_Error(
				'import_file_error',
				sprintf(
					__( 'Remote server returned %1$d %2$s for %3$s', 'overline' ),
					$code,
		...
		if ( isset( $headers['content-length'] ) && $filesize !== (int) $headers['content-length'] ) {
			unlink( $upload['file'] );
			return new \WP_Error( 'import_file_error', __( 'Remote file is incorrect size', 'overline' ) );
		}

		if ( 0 === $filesize ) {
			unlink( $upload['file'] );
			return new \WP_Error( 'import_file_error', __( 'Zero size file downloaded', 'overline' ) );
		}

		$max_size = (int) $this->max_attachment_size();
		if ( ! empty( $max_size ) && $filesize > $max_size ) {
			unlink( $upload['file'] );
			$message = sprintf( __( 'Remote file is too large, limit is %s', 'overline' ), size_format( $max_size ) );
		...
		foreach ( $todo as $post_id => $_ ) {
			$this->logger->debug(
				sprintf(
					...
					__( 'Running post-processing for post %d', 'overline' ),
					$post_id
				)
			);
			...
			if ( ! empty( $parent_id ) ) {
				...
				if ( isset( $this->mapping['post'][ $parent_id ] ) ) {
					...
					$this->logger->warning(
						sprintf(
							__( 'Could not find the post parent for "%1$s" (post #%2$d)', 'overline' ),
					...
					$this->logger->debug(
						sprintf(
							__( 'Post %1$d was imported with parent %2$d, but could not be found', 'overline' ),
							$post_id,
							$parent_id
						)
					);
			...
			if ( ! empty( $author_slug ) ) {
				...
				if ( isset( $this->mapping['user_slug'][ $author_slug ] ) ) {
					...
					$this->logger->warning(
						sprintf(
							__( 'Could not find the author for "%1$s" (post #%2$d)', 'overline' ),
							get_the_title( $post_id ),
							$post_id
						)
					);
					$this->logger->debug(
						sprintf(
							__( 'Post %1$d was imported with author "%2$s", but could not be found', 'overline' ),
							$post_id,
							$author_slug
						)
					);
			...
			if ( empty( $data ) ) {
				$this->logger->debug(
					sprintf(
						__( 'Post %d was marked for post-processing, but none was required.', 'overline' ),
						$post_id
					)
				);
				continue;
			...
			if ( is_wp_error( $result ) ) {
				$this->logger->warning(
					sprintf(
						__( 'Could not update "%1$s" (post #%2$d) with mapped data', 'overline' ),
						get_the_title( $post_id ),
						$post_id
					)
				);
	...
	protected function post_process_menu_item( $post_id ) {
		...
		if ( ! empty( $menu_object ) ) {
			...
			$this->logger->warning(
				sprintf(
					__( 'Could not find the menu object for "%1$s" (post #%2$d)', 'overline' ),
					get_the_title( $post_id ),
					$post_id
				)
			);
			$this->logger->debug(
				sprintf(
					__( 'Post %1$d was imported with object "%2$d" of type "%3$s", but could not be found', 'overline' ),
					$post_id,
					$menu_object_id,
					$menu_item_type
				)
		...
		foreach ( $todo as $comment_id => $_ ) {
			...
			if ( ! empty( $parent_id ) ) {
				...
				if ( isset( $this->mapping['comment'][ $parent_id ] ) ) {
					...
					$this->logger->warning(
						sprintf(
							__( 'Could not find the comment parent for comment #%d', 'overline' ),
							$comment_id
						)
					);
					$this->logger->debug(
						sprintf(
							__( 'Comment %1$d was imported with parent %2$d, but could not be found', 'overline' ),
							$comment_id,
							$parent_id
						)
					);
			...
			if ( ! empty( $author_id ) ) {
				...
				if ( isset( $this->mapping['user'][ $author_id ] ) ) {
					...
					$this->logger->warning(
						sprintf(
							__( 'Could not find the author for comment #%d', 'overline' ),
							$comment_id
						)
					);
					$this->logger->debug(
						sprintf(
							__( 'Comment %1$d was imported with author %2$d, but could not be found', 'overline' ),
							$comment_id,
							$author_id
						)
					);
			...
			if ( empty( $result ) ) {
				$this->logger->warning(
					sprintf(
						__( 'Could not update comment #%d with mapped data', 'overline' ),
						$comment_id
					)
				);
				continue;
	...
	protected function post_process_terms( $terms_to_be_remapped ) {
		...
		foreach ( $terms_to_be_remapped as $termid => $term_taxonomy ) {
			...
			if ( empty( $termid ) || ! is_numeric( $termid ) ) {
				$this->logger->warning(
					sprintf(
						__( 'Faulty term_id provided in terms-to-be-remapped array %s', 'overline' ),
						$termid
					)
				);
				continue;
			...
			if ( empty( $term_taxonomy ) ) {
				$this->logger->warning(
					sprintf(
						__( 'No taxonomy provided in terms-to-be-remapped array for term #%d', 'overline' ),
						$term_id
					)
				);
				continue;
			...
			if ( empty( $parent_slug ) ) {
				$this->logger->warning(
					sprintf(
						__( 'No parent_slug identified in remapping-array for term: %d', 'overline' ),
						$term_id
					)
				);
				continue;
			...
			if ( ! isset( $this->mapping['term_slug'][ $parent_slug ] ) || ! is_numeric( $this->mapping['term_slug'][ $parent_slug ] ) ) {
				$this->logger->warning(
					sprintf(
						__( 'The term(%1$d)"s parent_slug (%2$s) is not found in the remapping-array.', 'overline' ),
						$term_id,
						$parent_slug
					)
				);
			...
			if ( empty( $termattributes ) ) {
				$this->logger->warning(
					sprintf(
						__( 'No data returned by get_term_by for term_id #%d', 'overline' ),
						$term_id
					)
				);
				continue;
			...
			if ( is_wp_error( $result ) ) {
				$this->logger->warning(
					sprintf(
						__( 'Could not update "%1$s" (term #%2$d) with mapped data', 'overline' ),
						$termattributes['name'],
						$term_id
					)
				);
			...
			$this->logger->debug(
				sprintf(
					__( 'Term %1$d was successfully updated with parent %2$d', 'overline' ),
					$term_id,
					$mapped_parent
				)
			);
