...
class Helpers {
		...
		if ( ! $wp_filesystem->put_contents( $file_path, $content ) ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'An error occurred while writing file to your server! Tried to write a file to: %1$s%2$s.', 'overline' ),
					'<br>',
					$file_path
				)
			);
	...
	public static function data_from_file( $file_path ) {
		...
		if ( ! $data ) {
			return new \WP_Error(
				...
				sprintf(
					// translators: %1$s file path
					__( 'An error occurred while reading a file from your server! Tried reading file from path: %1$s%2$s.', 'overline' ),
					'<br>',
					$file_path
				)
			);
	...
	private static function check_wp_filesystem_credentials() {
		...
		if ( ! ( 'direct' === get_filesystem_method() ) ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'This WordPress page does not have %1$sdirect%2$s write file access. This plugin needs it in order to save the demo import xml file to the upload directory of your site. You can change this setting with these instructions: %3$s.', 'overline' ),
					'<strong>',
					'</strong>',
		...
		$plugin_page_setup = apply_filters(
			...
			array(
				...
				'page_title'  => esc_html__( 'One Click Demo Import', 'overline' ),
				'menu_title'  => esc_html__( 'Import Demo Data', 'overline' ),
				'capability'  => 'import',
		...
		if ( false === $creds ) {
			return new \WP_error(
				...
				__( 'An error occurred while retrieving reading/writing permissions to your server (could not retrieve WP filesystem credentials)!', 'overline' )
			);
		}
		...
		if ( ! WP_Filesystem( $creds ) ) {
			return new \WP_Error(
				...
				__( 'Your WordPress login credentials don\'t allow to use WP_Filesystem!', 'overline' )
			);
		}
	...
	public static function register_file_as_media_attachment( $log_path ) {
		...
		$attachment = array(
			...
			'post_title'     => apply_filters( 'pt-ocdi/attachment_prefix', esc_html__( 'One Click Demo Import - ', 'overline' ) ) . preg_replace( '/\.[^.]+$/', '', basename( $log_path ) ),
			'post_content'   => '',
			'post_status'    => 'inherit',
		);
	...
	public static function verify_ajax_call() {
		...
		if ( ! current_user_can( 'import' ) ) {
			wp_die(
				sprintf(
					// translators: %1$s html tags.
					esc_html__( '%1$sYour user role isn\'t high enough. You don\'t have permission to import demo data.%2$s', 'overline' ),
	...
	public static function process_uploaded_files( $uploaded_files, $log_file_path ) {
		...
		if ( $content_file_info && ! isset( $content_file_info['error'] ) ) {
			...
			$log_added = self::append_to_file(
				sprintf(
					// translators: %s error.
					__( 'Content file was not uploaded. Error: %s', 'overline' ),
					$widget_file_info['error']
				),
				$log_file_path,
				esc_html__( 'Upload files', 'overline' )
			);
		}
		...
		if ( $widget_file_info && ! isset( $widget_file_info['error'] ) ) {
			...
			$log_added = self::append_to_file(
				sprintf(
					// translators: %s error.
					__( 'Widget file was not uploaded. Error: %s', 'overline' ),
					$widget_file_info['error']
				),
				$log_file_path,
				esc_html__( 'Upload files', 'overline' )
		...
		if ( $customizer_file_info && ! isset( $customizer_file_info['error'] ) ) {
			...
			$log_added = self::append_to_file(
				sprintf(
					// translators: %s error.
					esc_html__( 'Customizer file was not uploaded. Error: %s', 'overline' ),
		...
		$log_added = self::append_to_file(
			esc_html__( 'The import files were successfully uploaded!', 'overline' ) . self::import_file_info( $selected_import_files ),
			$log_file_path,
			esc_html__( 'Upload files', 'overline' )
		);
	...
	public static function import_file_info( $selected_import_files ) {
		...
		return PHP_EOL .
			sprintf(
				...
				esc_html__( 'Initial max execution time = %s', 'overline' ),
				ini_get( 'max_execution_time' )
			) . PHP_EOL .
			sprintf(
				// translators: %s file names.
				__( 'Files info:%1$sSite URL = %2$s%1$sData file = %3$s%1$sWidget file = %4$s%1$sCustomizer file = %5$s%1$sRedux files:%1$s%6$s', 'overline' ),
				PHP_EOL,
				get_site_url(),
				empty( $selected_import_files['content'] ) ? esc_html__( 'not defined!', 'overline' ) : $selected_import_files['content'], // phpcs:ignore
				empty( $selected_import_files['widgets'] ) ? esc_html__( 'not defined!', 'overline' ) : $selected_import_files['widgets'], // phpcs:ignore
				empty( $selected_import_files['customizer'] ) ? esc_html__( 'not defined!', 'overline' ) : $selected_import_files['customizer'], // phpcs:ignore
				empty( $redux_file_string ) ? esc_html__( 'not defined!', 'overline' ) : $redux_file_string // phpcs:ignore
			);
	}
