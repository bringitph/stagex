...
function thb_password_form() {
	...
	<p class="password-text">' . esc_html__( 'This is a protected area. Please enter your password:', 'overline' ) . '</p>
	<input name="post_password" type="password" class="box" placeholder="' . esc_html__( 'Password', 'overline' ) . '"/><input type="submit" name="Submit" class="btn" value="' . esc_attr__( 'Submit', 'overline' ) . '" /></form>
	';
	return $o;
}
				...
				<div class="page-title">
					<h1>
						...
						if ( is_category() ) {
							...
							printf( esc_html__( 'Search Results for: %s', 'overline' ), '<span>' . get_search_query() . '</span>' );

						} elseif ( is_author() ) {
							/* translators: %s: author name */
							printf( esc_html__( 'Author Archives: %s', 'overline' ), '<span>' . get_the_author() . '</span>' );

						} elseif ( is_day() ) {
							/* translators: %s: day */
							printf( esc_html__( 'Daily Archives: %s', 'overline' ), '<span>' . get_the_date() . '</span>' );

						} elseif ( is_month() ) {
							/* translators: %s: month */
							printf( esc_html__( 'Monthly Archives: %s', 'overline' ), '<span>' . get_the_date( 'F Y' ) . '</span>' );

						} elseif ( is_year() ) {
							/* translators: %s: year */
							printf( esc_html__( 'Yearly Archives: %s', 'overline' ), '<span>' . get_the_date( 'Y' ) . '</span>' );

						} elseif ( is_home() ) {
							esc_html_e( 'Latest News', 'overline' );
						}
						?>
					</h1>
					<?php
