(function($, window) {
  ...
  SITE = {
    ...
    animatedMenuItem: {
      selector: '.thb-animated-menu-item',
      init: function() {
        var base = this,
          container = $(base.selector);
    ...
    slick: {
      selector: '.thb-carousel, .up-sells .products, .related .products, .cross-sells .products',
      init: function(el) {
        var base = this,
        ...
        container.each(function() {
          ...
          if (_this.parents('.up-sells.products').length || _this.parents('.related.products').length) {
            columns = 4;
            navigation = true;

            _this.addClass('thb-carousel thb-product-carousel');
          }
          if (_this.parents('.cross-sells').length) {
            columns = 3;
            navigation = true;
          ...
          if (_this.parents('.cross-sells').length) {
            args.responsive[1].settings.slidesToShow = 3;
          }
    ...
    autoComplete: {
      ...
      init: function() {
        ...
        $('.thb-quick-search .thb-item-icon-wrapper', '.header').on('click', function(e) {
          header.toggleClass('thb-search-active');
          if (header.hasClass('thb-search-active')) {
            header.find('.search-field').focus();
          }
          e.stopPropagation();
        });

        win.on('resize.thb-remove-search', function() {
          header.removeClass('thb-search-active');
        });
          ...
          field.autocomplete({
            ...
            onSearchComplete: function(query, suggestions) {
              ...
              if (suggestions.length) {
                $('.thb-results-container').append($('<div class="thb-search-btn"><a href="' + themeajax.settings.site_url + '?s=' + query + '&post_type=product" class="btn style2 black">' + themeajax.l10n.results_all + '</a></div>'));
              }
            }
          });
        });
    ...
    productStyle3: {
      ...
      init: function() {
        var base = this,
          container = $(base.selector),
          thumbnail_container = $('<div class="product-thumbnails" />').appendTo(container),
          image_container = $('.woocommerce-product-gallery__wrapper', container),
        ...
        thumbnail_container.wrapInner("<div class='thb-fixed'></div>");

        var thumbnails = $('.woocommerce-product-gallery__image', thumbnail_container);
